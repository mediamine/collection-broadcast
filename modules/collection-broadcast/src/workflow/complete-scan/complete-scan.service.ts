    ...
    try {
      ...
      for (const [, newsItem] of existingNewsItemHashWithNoPageText.entries()) {
        ...
        if (link && isCompleteScanExcludedConditions(link)) {
          try {
            const { description, audioSource, text } = await feedScraperService.scanArticle({ page, url: link });

            const audioSourceText = (await this.assemblyAiService.transcribe({ audio: audioSource }))?.text;
            this.logger.log(
            ...
            await this.prismaService.news_item.update({
              where: { id },
              data: { description, page_text: audioSourceText }
            });
          } catch (e) {
